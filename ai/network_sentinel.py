...
network_sentinel.py
-------------------
A beginner-friendly network security monitoring tool with a live terminal dashboard.
Detects: Port scans and connections to/from known malicious IPs.

Requirements:
    pip install scapy requests colorama rich

Usage:
    sudo python network_sentinel.py                  # monitor all interfaces
    sudo python network_sentinel.py -i eth0          # specific interface
    sudo python network_sentinel.py --pcap file.pcap # read from a .pcap file
...
# Rich — the terminal dashboard library
...
PORT_SCAN_WINDOW    = 10   # seconds window for port scan detection

HARDCODED_BAD_IPS = {
    "198.51.100.1",   # TEST-NET demo IP
    "203.0.113.42",   # TEST-NET-3 demo IP
    "91.109.178.5",
}

# Maximum alerts to show on screen (older ones scroll off)
...
class ThreatFeed:
    """Downloads and auto-refreshes a live malicious IP blocklist."""
...
# =============================================================================
# DASHBOARD — built with Rich
# =============================================================================

def build_dashboard(state: DetectorState) -> Layout:
    """
    Builds the full terminal dashboard layout.
    Called every refresh cycle by Rich's Live context.

    Layout:
    ┌─────────────────────────────────────────┐
    │              HEADER / TITLE             │
    ├────────────────────┬────────────────────┤
    │   STATS (left)     │  TOP TALKERS(right)│
    ├────────────────────┴────────────────────┤
    │              ALERT LOG                  │
    ├─────────────────────────────────────────┤
    │         THREAT FEED STATUS              │
    ...
    # ── Header ────────────────────────────────────────────────────────────────
    header_text = Text()
    header_text.append("⚡ NETWORK SENTINEL", style="bold bright_red")
    ...
    feed_text.append("Log: logs/sentinel.log", style="dim white")
    feed_text.append("   |   ", style="dim white")
...
def main():
    ...
    # ── Startup ───────────────────────────────────────────────────────────────
    console.print("\n[bold red]⚡ NETWORK SENTINEL[/] — starting up...\n")
    ...
    # Log startup info
    state.add_alert("INFO", f"Sentinel started — feed: {threat_feed.status}")
    ...
    # ── Launch sniffer in background thread ───────────────────────────────────
    def start_sniffing():
        if args.pcap:
    ...
    # ── Live dashboard loop ───────────────────────────────────────────────────
    # Rich's Live context redraws the layout every `refresh_per_second` times/sec
    try:
        with Live(
