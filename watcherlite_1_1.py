...
# a local network.
# This version has been updated from version 1.0.
...
class DetectorState:
	...
	def __init__(self, malicious_ips: set, threat_feed: ThreatFeed):
		...
		# UDP scan tracking
		self.udp_port_hits: dict       = defaultdict(dict)  # direct method: { src_ip: { port: timestamp } }
		self.icmp_hits: dict           = defaultdict(dict)  # ICMP method:   { scanner_ip: { timestamp: True } }
		self.alerted_udp_scanners: set = set()
	...
	def reset_old_udp_hits(self, src_ip: str):
		"""Remove UDP port hits outside the time window."""
		now = time.time()
		self.udp_port_hits[src_ip] = {
			p: ts for p, ts in self.udp_port_hits[src_ip].items()
			if now - ts <= PORT_SCAN_WINDOW
		}

	def reset_old_icmp_hits(self, scanner_ip: str):
		"""Remove ICMP hits outside the time window."""
		now = time.time()
		self.icmp_hits[scanner_ip] = {
			ts: v for ts, v in self.icmp_hits[scanner_ip].items()
			if now - ts <= PORT_SCAN_WINDOW
		}
...
def detect_udp_scan(packet, state: DetectorState):
	...
	state.udp_port_hits[src_ip][dst_port] = time.time()
	state.reset_old_udp_hits(src_ip)

	unique_ports = len(state.udp_port_hits[src_ip])

	if unique_ports >= PORT_SCAN_THRESHOLD and src_ip not in state.alerted_udp_scanners:
		msg = (f"UDP scan (direct) from {src_ip} — "
			   f"{unique_ports} unique ports in {PORT_SCAN_WINDOW}s")
		state.add_alert("ALERT", msg)
		state.alerted_udp_scanners.add(src_ip)
	elif unique_ports < PORT_SCAN_THRESHOLD // 2:
		state.alerted_udp_scanners.discard(src_ip)
...
def handle_packet(packet, state: DetectorState):
	...
	detect_port_scan(packet, state)		# TCP SYN scans  (nmap -sS)
	detect_udp_scan(packet, state)		# UDP direct     (nmap -sU)
...
def build_dashboard(state: DetectorState) -> Layout:
	...
	# Header
	header_text = Text()
	header_text.append("⚡ Watcher Lite 1.1 NEW!", style="bold bright_red")
	...
	stats_table.add_row("TCP Scanners",      f"{len(state.alerted_scanners)}")
	stats_table.add_row("UDP Scanners",      f"{len(state.alerted_udp_scanners)}")
	...
	# Highlight IPs that have been flagged
		ip_style = "bold red" if ip in state.alerted_bad_ips or ip in state.alerted_scanners or ip in state.alerted_udp_scanners else "white"
	...
	feed_text.append("   |   ", style="dim white")
	feed_text.append("Press Ctrl+C to exit", style="dim white")
